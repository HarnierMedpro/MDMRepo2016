            ...
            if (ModelState.IsValid)
            {
                using (DbContextTransaction dbTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            tableColumnInfos.Add(new TableInfo { Field_ColumName = "CollNoteType",NewValue = actionCode.CollNoteType,OldValue = toStore.CollNoteType});
                            toStore.CollNoteType = actionCode.CollNoteType;
                        }
                        if (toStore.CodeID != actionCode.CodeID)
                        {
                            tableColumnInfos.Add(new TableInfo{Field_ColumName = "CodeID",NewValue = actionCode.CodeID.ToString(),OldValue = toStore.CodeID.ToString()});
                            toStore.CodeID = actionCode.CodeID;
                        }
                        if (toStore.CategoryID != actionCode.CategoryID)
                        {
                            tableColumnInfos.Add(new TableInfo{Field_ColumName = "CategoryID",NewValue = actionCode.CategoryID.ToString(),OldValue = toStore.CategoryID.ToString()});
