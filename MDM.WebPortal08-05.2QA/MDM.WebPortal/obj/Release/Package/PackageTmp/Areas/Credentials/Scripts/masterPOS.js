...
function sendPhyGrp(t) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if ($(".addPhy_"+t).length) {
                console.log("Exist btn add phy group");
                $(".addPhy_"+t).hide();
            } else {
                console.log("Doesn't exist btn add phy group");
            }

            if ($(".expPhy_"+t).length) {
                console.log("Exist btn export phy group");
                $(".expPhy_"+t).show();
            } else {
                console.log("Doesn't Exist btn export phy group");
            }
            //location.reload();
        },
...
function onChangeAdInfo(e) {
    if (e.response.Data.length > 0) {
        var currentMasterPos = e.response.Data[0].MasterPOSID;

        if (e.type !== "destroy" ) {
            console.log("Funciones");
            if ($(".addInfo_" + currentMasterPos).length) {
                console.log("add button exist");
                $(".addInfo_" + currentMasterPos).hide();
            } else {
                console.log("add button does not exist");
            }
            if ($(".exp_" + currentMasterPos).length) {
                console.log("Existe Export");
                $(".exp_" + currentMasterPos).show();
            } else {
                console.log("No existe Export");
...
function onChangeLic(e) {
    if (e.response.Data.length > 0) {
        var currentMasterPos = e.response.Data[0].MasterPOSID;
        if (e.type !== "destroy") {
            if ($(".addLic_" + currentMasterPos).length) {
                console.log("Existe add");
                $(".addLic_" + currentMasterPos).hide();
            } else {
                console.log("No existe add");
            }
            if ($(".expLic_" + currentMasterPos).length) {
                console.log("Existe Export");
                $(".expLic_" + currentMasterPos).show();
            } else {
                console.log("No existe Export");
...
function onChangeFacInfo(e) {
    if (e.response.Data.length > 0) {
        var currentMasterPos = e.response.Data[0].MasterPOSID;
        if (e.type !== "destroy") {
            if ($(".addFI_" + currentMasterPos).length) {
                console.log("Existe add");
                $(".addFI_" + currentMasterPos).hide();
            } else {
                console.log("No existe add");
            }
            if ($(".expFI_" + currentMasterPos).length) {
                console.log("Existe Export");
                $(".expFI_" + currentMasterPos).show();
            } else {
                console.log("No existe Export");
...
function onChangeExtraInfoD(e) {
    if (e.response.Data.length > 0) {
        var currentMasterPos = e.response.Data[0].MasterPOSID;
        if (e.type !== "destroy") {
            if ($(".btnAddInf_" + currentMasterPos).length) {
                console.log("Exist btn add extra info detail");
                $(".btnAddInf_" + currentMasterPos).hide();
            } else {
                console.log("Doesn't exist btn add extra info detail");
            }
            if ($(".btnExpInf_" + currentMasterPos).length) {
                console.log("Exist btn export extra info detail");
                $(".btnExpInf_" + currentMasterPos).show();
            } else {
                console.log("Doesn't Exist btn export extra info detail");
            }

            var tabStrip = $("#tabExtData_" + currentMasterPos).kendoTabStrip().data("kendoTabStrip");
            console.log(tabStrip.items());
            var tabToEnable = $("li.k-item.k-state-disabled.k-last");
            tabStrip.enable(tabToEnable, true);
        }
    }
}

function onChangePhyG(e) {
    console.log(e.response.Data.length);
    if (e.response.Data.length > 0) {
        var currentMasterPos = e.response.Data[0].MasterPOSID;
        if (e.type !== "destroy" && currentMasterPos !== undefined) {
            if ($(".addPhy_" + currentMasterPos).length) {
                console.log("Exist btn add phy group");
                $(".addPhy_" + currentMasterPos).hide();
            } else {
                console.log("Doesn't exist btn add phy group");
            }

            if ($(".expPhy_" + currentMasterPos).length) {
                console.log("Exist btn export phy group");
                $(".expPhy_" + currentMasterPos).show();
            } else {
                console.log("Doesn't Exist btn export phy group");
            }
        }
